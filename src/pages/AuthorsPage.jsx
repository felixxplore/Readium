...
export default function AuthorsPage({ onNavigate }) {
  ...
  const categories = ['ALL', ...new Set(AUTHORS_DATA.map((author) => author.category))]
  ...
  return (
    <div className="min-h-screen bg-[#f9f9f9]">
      ...
      <main className="max-w-7xl mx-auto px-6 py-16">
        <div className="mb-12">
          <h1 className="font-serif text-4xl md:text-5xl mb-4 text-black" style={{ fontFamily: 'var(--font-serif)' }}>
            Our Contributors
          </h1>
          <p className="text-lg text-[#666] max-w-2xl">
            Meet the writers, thinkers, and creators behind Lumen Editorial. Each brings a unique perspective to contemporary culture, design, and ideas.
          </p>
        </div>
                ...
                <div className="mb-4 rounded-sm overflow-hidden bg-gray-200 aspect-square">
                  {author.image ? (
                    ...
                    <div className="w-full h-full bg-gradient-to-br from-[#f3f3f3] to-[#e8e8e8] flex items-center justify-center">
                      <span className="text-4xl font-serif text-gray-400">{author.name.charAt(0)}</span>
                    </div>
                  )}
                </div>

                <div className="mb-4">
                  <h3 className="font-serif text-xl text-black mb-1" style={{ fontFamily: 'var(--font-serif)' }}>
                    {author.name}
                  </h3>
                  ...
                  <div className="mb-3">
                    <span
                      ...
                      style={{ backgroundColor: getCategoryColor(author.category) }}
                    >
                      {author.category}
                    </span>
                  </div>

                  <p className="text-sm text-gray-600 leading-relaxed line-clamp-3 mb-4">{author.bio}</p>
                </div>

                <div className="flex items-center justify-between pt-4 border-t border-black/10">
                  <div>
                    <p className="text-xs uppercase tracking-widest font-medium text-gray-500">Posts</p>
                    <p className="text-lg font-semibold text-black">{author.postCount}</p>
                  </div>
                  <div>
                    <p className="text-xs uppercase tracking-widest font-medium text-gray-500">Followers</p>
                    <p className="text-lg font-semibold text-black">{author.followCount.toLocaleString()}</p>
                  </div>
                </div>
